...
export default async function handler(req: any, res: any) {
  ...
  await enviarCorreo(correo, nombre,
    ...
    `<div style="font-family:'Segoe UI',sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#f9f8ff;border-radius:16px;">
      ...
      <div style="background:white;border-radius:12px;padding:20px;margin:20px 0;border:1.5px solid #ede9f8;">
        <div style="margin-bottom:10px;"><span style="color:#7b6fa0;font-size:13px;">Psicóloga</span><br/><strong>${psicologa}</strong></div>
      ...
      <p style="color:#7b6fa0;font-size:13px;">Si necesitas cancelar con más de 24 horas, puedes hacerlo desde la app. Si es con menos de 24 horas, escríbenos a <a href="mailto:bienestarysaludmental@uft.cl" style="color:#3d2f7a;">bienestarysaludmental@uft.cl</a>.</p>
      <p style="color:#a89ec0;font-size:12px;margin-top:24px;text-align:center;">Bienestar y Salud Mental UFT</p>
    </div>`
  );

  // 2. Correo a bienestar
  await enviarCorreo(CORREO_BIENESTAR, 'Bienestar UFT',
  ...
  if (correoPsicologa) {
    await enviarCorreo(correoPsicologa, psicologa,
